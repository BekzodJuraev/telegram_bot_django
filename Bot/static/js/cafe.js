(function($) {
  ...
  var Cafe = {
    ...
    init: function(options) {
      ...
      Cafe.deliveryOption=options.deliveryOption;
      Cafe.user_longitude=options.user_longitude;
      Cafe.user_latitude=options.user_latitude;
    ...
    formatPrice: function(price) {
      return 'uzs' + Cafe.formatNumber(price);
    },
    ...
    getOrderData: function() {
      ...
      $('.js-item').each(function() {
        var itemEl = $(this)
        var add_item = itemEl.data('item-id');
        var count = +itemEl.data('item-count') || 0;
        if (count > 0) {
          order_data.push({'add_item':add_item, 'count':count});
        }
      });
      return order_data;
    },
    ...
    mainBtnClicked: function() {
      ...
      if (Cafe.modeOrder) {
        ...
        var adress = $('.js-order-comment-field_address').val();
        var phone = $('.js-order-comment-field_phone').val();
...
apiRequest: function(method, data, onCallback) {
  ...
  var orderItem = Cafe.getOrderData();

  // Check if Cafe.adress is empty or null


  // Extend the data object with additional properties
  data = $.extend(data, {
    "name": Telegram.WebApp.initDataUnsafe.user.first_name,
    "price": Cafe.totalPrice,
    "id_user": Telegram.WebApp.initDataUnsafe.user.id,
    "comment": Cafe.comment,
    "adress": Cafe.adress,
    "phone": Cafe.phone,
    "related_diaries": orderItem,
    "latitude":Cafe.user_latitude,
    "longitude":Cafe.user_longitude,
    "order_choice":Cafe.deliveryOption
  });
